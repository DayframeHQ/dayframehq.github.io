...
describe('progressive personalization', () => {
  ...
  it('uses a calm two-card default for broad interests', () => expect(focusCategories(['everything'])).toEqual(['Train', 'Study']))
  it('ranks route, scheduled work, interests, then remaining categories', () => expect(rankQuickAddCategories('/study', ['life'], ['train'])).toEqual(['Study', 'Train', 'Life', 'Health']))
