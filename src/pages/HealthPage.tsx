  ...
  return (
    <div className="page">
      ...
      {labs.length > 0 ? <section className="card card-pad section">
        ...
        <div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><LineChart data={selected}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--line)" /><XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: 'var(--muted)', fontSize: 11 }} /><YAxis domain={['dataMin - 5', 'dataMax + 5']} hide /><Tooltip contentStyle={{ borderRadius: 12, border: '1px solid var(--line)', background: 'var(--surface)' }} /><Line dataKey="result" stroke="#25935a" strokeWidth={2.5} dot={{ r: 5, fill: '#25935a' }} /></LineChart></ResponsiveContainer></div>
