...
export async function createQuickWorkoutSession(identity:RepoIdentity,input:{planId:string;scheduledDate:string;movements:TrainExerciseOption[]}){
  ...
  const sessionId=crypto.randomUUID();const estimatedMinutes=input.movements.reduce((sum,item)=>sum+(item.tracking==='duration'?(item.minutes??20):item.sets*3),0);const title=input.movements.length===1?input.movements[0].name:`Quick workout · ${input.movements.length} movements`
  ...
  if(identity.isDemo){const state=readDemo();const session:PlannedSession={id:sessionId,plan_id:input.planId,domain:'train',title,description:'Built from the Workouts quick-start movement library.',scheduled_date:input.scheduledDate,estimated_minutes:estimatedMinutes,status:'planned',planned_items:plannedItems};writeDemo({...state,sessions:[session,...state.sessions]});return session}
  const client=clientFor(identity)!;const created=await client.from('planned_sessions').insert({user_id:identity.user!.id,plan_id:input.planId,domain:'train',title,description:'Built from the Workouts quick-start movement library.',scheduled_date:input.scheduledDate,estimated_minutes:estimatedMinutes,status:'planned'}).select('*').single();if(created.error)throw new Error(created.error.message)
